...
export const fetchGitHubUsers = async (since: number = 0): Promise<GitHubUser[]> => {
...
export const searchGitHubUsers = async (query: string, page: number = 1): Promise<{ items: GitHubUser[]; total_count: number }> => {
  if (!query.trim()) {
